...
def pyvcf_read_vcf(path, **kwargs):
    ...
    with open(path) as handle:
        ...
        fmt_col = (n for n, f in enumerate(fields) if f == 'FORMAT').next()
        sample_cols = fields[fmt_col+1:]
        data = []
        for row in reader:
            ...
            for sample in sample_cols:
                ...
                for anum, call_num in enumerate(genotype_info['GT']):
                    if call_num != '.':
                        call = calls[int(call_num)]
                        is_ref = True if call_num == '0' else False
                        data.append({'Allele': anum,
                                     'ALT': call,
                                     'IsRef': is_ref,
                                     'POS': row['POS'],
                                     'REF': row['REF'],
